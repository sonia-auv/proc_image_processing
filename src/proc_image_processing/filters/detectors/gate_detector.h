...
namespace proc_image_processing
{
    ...
    void apply(cv::Mat &image) override
    {
      ...
      if (num_of_objects > 1)
      {
        if (vote_most_centered_())
        {
          ...
          cv::circle(output_image_, cv::Point(objVec[0]->GetCenter().x, objVec[0]->GetCenter().y) - cv::Point(12, 12),
                     6, CV_RGB(255, 0, 0), -1);
        }
        ...
        if (vote_length_())
        {
          ...
          cv::circle(output_image_, cv::Point(objVec[0]->GetCenter().x, objVec[0]->GetCenter().y) - cv::Point(12, 6),
                     6, CV_RGB(0, 0, 255), -1);
        }
        ...
        if (vote_most_upright_())
        {
          ...
          cv::circle(output_image_, cv::Point(objVec[0]->GetCenter().x, objVec[0]->GetCenter().y) - cv::Point(12, 0),
        ...
        cv::circle(output_image_, cv::Point(objVec[0]->GetCenter().x, objVec[0]->GetCenter().y) - cv::Point(12, -18),
  ...
  private:
    ...
    Parameter<bool> debug_contour_, use_convex_hull_, offset_y_for_fence_;
