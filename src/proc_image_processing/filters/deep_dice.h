...
private:
    ...
    inline void drawTarget(cv::Mat &image, const Detection &object, int thickness=3, const cv::Scalar &color_box=cv::Scalar(0, 255, 0)){
        ...
        cv::Rect rect_top(origin_x, origin_y-20, (int)object.bbox.size_x, origin_y);
        ...
        cv::putText(image, text, cv::Point(origin_x,origin_y - 20), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0,0,0), 2, CV_AA);
    }
